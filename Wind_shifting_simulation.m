...
[p1(jj), p2(jj), TT(jj)] = Anti_wind_parameter_v3(v0, Dx, Dy, gama, d); % Anti_wind_parameter_v3.m output the shifting parameters.
end
...
vx0 = p1(jj)*v0*cos(alpha)*cos(p2(jj)*gama);
vy0 = p1(jj)*v0*cos(alpha)*sin(p2(jj)*gama);
vz0 = p1(jj)*v0*sin(alpha);
...
pause(0.001)
subplot(2,2,1:2);
title({['11 MPH wind after shifting']}, 'FontSize', 15);
